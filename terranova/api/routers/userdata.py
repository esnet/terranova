...
elastic_backend = ElasticSearchBackend()
...
def get_userdata(user: User = Security(auth_check, scopes=[TOKEN_SCOPES["read"]])) -> UserData:
    result = elastic_backend.get_userdata(user)
    if len(result) < 1:
...
def create_userdata(
    ...
    result = elastic_backend.create_userdata(userdata=new_data, user=user)
    return result
...
def update_userdata(
    ...
    try:
        return elastic_backend.update_userdata(userdata=update_data, user=user)
