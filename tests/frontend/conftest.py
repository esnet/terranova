...
def fresh_db():
    """
    Delete test DB files once per session to ensure a clean initial state.
    Runs before any test server is started.
    SQLAlchemy-based basic_auth.sqlite can accumulate corrupt/stale user rows
    across test runs, causing IndexError in the auth layer.
    """
    parent_dir = os.path.dirname(__file__)
    app_root = os.path.abspath(os.path.join(parent_dir, os.pardir, os.pardir))
    db_paths = [
        os.path.join(app_root, "terranova-test.db"),
        "/var/tmp/basic_auth.sqlite",
    ]
...
def setup_server_processes(request):
    """
    Spawn a frontend and backend process once per session (not per test).
    Session scope means ~1.5s startup cost instead of 2s * N tests.
    """
    ...
    # Poll until both servers are ready (typically <500ms) instead of sleeping flat
    for _url in [FRONTEND_BASE + "/", API_BASE + "/current_user/"]:
        for _ in range(30):
            try:
                requests.get(_url, timeout=0.5)
                break
            except Exception:
                time.sleep(0.2)
